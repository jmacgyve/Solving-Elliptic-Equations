...
//-----------------------------------
// Два метода решения Jacobi, SOR
//-----------------------------------
void Jacobi(double **p,double dx, double dy, double tol, double *tot_time, int *iter,int BC);
...
double func(int i,int j,double dx,double dy)
{
    return (-(2.25*pi*pi)*(i*dx*i*dx + j*dy*j*dy)*sin(2*pi*i*dx*j*dy));
}
